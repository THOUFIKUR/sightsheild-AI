...
t = tic;
try
    % Run calibration with no data (framework demonstration, clearly labeled)
    result.calibration = calibrate_confidence();
catch ex
    fprintf('[Calibration ERROR] %s\\n', ex.message);
    result.calibration = struct('error', ex.message, ...
        'validation_note', 'Calibration module failed to execute.');
end
...
if isfield(result.hemorrhages, 'total_count')
