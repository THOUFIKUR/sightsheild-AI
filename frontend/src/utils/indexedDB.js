    ...
    upgrade(db, oldVersion) {
      ...
      if (oldVersion < 5) {
        if (!db.objectStoreNames.contains("audit_log")) {
          db.createObjectStore("audit_log", {
            keyPath: "id",
            autoIncrement: true,
          }).createIndex("ts", "ts");
        }
...
export async function savePatient(patientRecord) {
  ...
  await db.put(STORE, patientRecord);
  ...
  try {
    ...
    await enqueueRequest({
      url: "SUPABASE_INSERT_PATIENT", // symbolic (we'll handle manually)
      method: "SUPABASE",
      body: patientRecord,
    });
  }
